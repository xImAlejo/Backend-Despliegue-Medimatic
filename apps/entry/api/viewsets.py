...
class EntryViewSets(viewsets.ModelViewSet):
    ...
    def update(self, request, pk=None):
        try:
            ...
            return Response({'message': 'Los Entries que intenta actualizar no existe'}, status=status.HTTP_404_NOT_FOUND)
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        try:
            ...
            user.delete()
            return Response({'message': 'Entry eliminado correctamente'}, status=status.HTTP_200_OK)
        except self.model.DoesNotExist:
            return Response({'message': 'Entry no encontrado'}, status=status.HTTP_404_NOT_FOUND)

    @action(detail=True, methods=['get'])
    def getEntriesbySerieId(self,request,pk=None):
       
        self.queryset = self.serializer_class().Meta.model.objects.filter(serie_id=pk)
        entries = self.get_queryset()
        entries_serializer = self.serializer_class(entries, many=True)
        data = {
            
            "total": self.get_queryset().count(),
            "rows": entries_serializer.data
        }
        return Response(data, status=status.HTTP_200_OK)
    ...
    def UpdateQuantitybySerieId(self, request, pk=None):
        try:
            entry = self.serializer_class().Meta.model.objects.get(serie_id=pk)
        except Entry.DoesNotExist:
            return Response({'message': 'Entry no encontrado'}, status=status.HTTP_404_NOT_FOUND)
        ...
        try:
            ...
            entry.save()
            return Response({'quantity': entry.quantity}, status=status.HTTP_200_OK)
        except ValueError:
